...
//        PeakDim.setResonanceFactory(new AtomResonanceFactory());
    ...
    public void buildAcquisitions() {
        for (Acquisition acquisition : UmbcProject.getActive().acquisitionTable) {
            acquisition.setupListeners();
        }
    }

    public void buildPeakConstraintLinks() throws ParseException {
        Iterator iter = star3.getSaveFrames().values().iterator();
        while (iter.hasNext()) {
            Saveframe saveframe = (Saveframe) iter.next();
            if (saveframe.getCategoryName().equals("peak_constraint_links")) {
                if (DEBUG) {
                    System.err.println("process peak_constraint_links " + saveframe.getName());
                }
    ...
    public void buildSamples() throws ParseException {
        for (Saveframe saveframe : star3.getSaveFrames().values()) {
            if (saveframe.getCategoryName().equals("sample")) {
                if (DEBUG) {
                    System.err.println("process sample " + saveframe.getName());
                }
                processSTAR3Sample(saveframe);
            }
        }
    }

    public void buildConditions() throws ParseException {
        for (Saveframe saveframe : star3.getSaveFrames().values()) {
            if (saveframe.getCategoryName().equals("sample_conditions")) {
                if (DEBUG) {
                    System.err.println("process condition " + saveframe.getName());
                }
                processSTAR3Condition(saveframe);
            }
        }
    }
    ...
    public void buildPeakLists() throws ParseException {
        ...
        for (Saveframe saveframe : star3.getSaveFrames().values()) {
            if (saveframe.getCategoryName().equals("spectral_peak_list")) {
                ...
                String experimentClass = saveframe.getOptionalValue("_Spectral_peak_list","Experiment_class");
                if (!experimentClass.equals("")) {
                    processSTAR3ManagedList(saveframe);
                } else {
    ...
    public void processSTAR3Sample(Saveframe saveframe) throws ParseException {
        ...
        if (loop != null) {
            ...
            for (int i = 0; i<entityLabels.size(); i++) {
                ...
                sample.setEntityLabelString(entity,entityLabelString.get(i).replace("^'", "").replace("'$",""));
            }
        }
    }

    public void processSTAR3Condition(Saveframe saveframe) throws ParseException {
        String conditionName = saveframe.getValue("_Sample_condition_list", "Name").replace("^'", "").replace("'$","");
        String conditionDetails = saveframe.getValue("_Sample_condition_list", "Details").replace("^'", "").replace("'$","");

        Condition condition=new Condition(conditionName);
        condition.setDetails(conditionDetails);
        ...
        if (loop != null) {
            ...
            for (int i = 0; i<typeLabels.size(); i++) {
                condition.setVariable(typeLabels.get(i),Double.parseDouble(valString.get(i)),Double.parseDouble(valErrString.get(i)));
            }
        }
    }
    ...
    public void processSTAR3ManagedList(Saveframe saveframe) throws ParseException {
        ...
        if (loop != null) {
            ...
            for (int i = 0, n = idColumn.size(); i < n; i++) {
                ...
                peakList.loadPeak(peak);  // old code added without creating resonance, but that caused problems with new
                // linking resonance code used here
            }
...
//                    }
    ...
    public void processPeakConstraintLinks(Saveframe saveframe) throws ParseException {
         ...
         */
        List<String> constraintIDColumn = loop.getColumnAsList("Constraint_ID");
        List<String> peakIDColumn = loop.getColumnAsList("Peak_ID");
        List<String> listNameColumn = loop.getColumnAsList("Spectral_Peak_list_Sf_framecode");

        ManagedList list=null;
        NoeSet noeSet = NoeSet.getSet(noeSetName);

        if (noeSet==null) {
            noeSet=NoeSet.addSet(noeSetName);
        }

        for (int i = 0; i < constraintIDColumn.size(); i++) {
            if (list == null) {
                list = (ManagedList) PeakList.get(listNameColumn.get(i));
                list.setNoeSet(noeSet);
            }
            try {
                ManagedPeak peak = (ManagedPeak) list.getPeakByID(Integer.parseInt(peakIDColumn.get(i)));
                Noe noe = noeSet.getByStarId(Integer.parseInt(constraintIDColumn.get(i)));
                if (noe != null) {
                    peak.getNoes().add(noe);
                }
            } catch (Exception e) {
                System.out.println("Parsing Peak Constraint List: "+list);
            }
        }
    ...
    public void processGenDistConstraints(Saveframe saveframe) throws ParseException {
        ...
        String name=saveframe.getOptionalValue("_Gen_dist_constraint_list","Name");
        String setID=saveframe.getOptionalValue("_Gen_dist_constraint_list","ID");
        Integer setIdNum=Integer.parseInt(setID);
        if (name.equals("")) {
            name=saveframe.getName().substring(5);
        }
        NoeSet noeSet = NoeSet.addSet(name);
        if (setIdNum!=null) {
            noeSet.setIdNum(Integer.parseInt(setID));
        }
        for (int i = 0; i < entityAssemblyIDColumns[0].size(); i++) {
    ...
    public void process(String[] argv) throws ParseException, IllegalArgumentException {
        ...
        if (argv.length == 0) {
            ...
            buildSamples();
            if (DEBUG) {
                System.err.println("process conditions");
            }
            buildConditions();
            if (DEBUG) {
            ...
            buildRDCConstraints();
            if (DEBUG) {
                System.err.println("process peak constraint links");
            }
            buildPeakConstraintLinks();
            if (DEBUG) {
                System.err.println("build acquisitions");
            }
            buildAcquisitions();
            if (DEBUG) {
